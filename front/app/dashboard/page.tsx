    ...
    return (
      <DashboardLayout>
        <div className="flex flex-col items-center justify-center h-[80vh]">
          ...
          <p className="text-lg text-slate-500 font-medium animate-pulse">Chargement de votre espace...</p>
        </div>
      </DashboardLayout>
    );
  }
      ...
      <DashboardLayout>
        <div className="flex flex-col items-center justify-center h-[80vh]">
          ...
          <button
            ...
            className="px-6 py-3 bg-indigo-600 text-white rounded-xl font-medium hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-500/20"
          >
            Réessayer
          </button>
        </div>
      ...
      <div className="space-y-10 animate-fade-in">
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
          <div>
            ...
            <p className="text-slate-500 mt-2 text-lg">
              Bienvenue sur votre espace de gestion <span className="font-semibold text-indigo-600">Net Strategy</span>.
            </p>
          </div>

          <button
            onClick={() => window.location.reload()}
            className="group flex items-center gap-2 bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 font-semibold px-5 py-2.5 rounded-xl transition-all shadow-sm hover:shadow-md"
          >
          ...
          <section className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
            <div className="p-8 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">Administrez les accès et les rôles de l'équipe.</p>
                </div>
              </div>
              <a
                href="/users/create"
                className="bg-slate-900 hover:bg-slate-800 text-white px-6 py-3 rounded-xl font-semibold shadow-lg shadow-slate-900/20 transition-all active:scale-95 text-sm flex items-center gap-2"
              >
            ...
            {users.length === 0 ? (
              <div className="p-16 text-center">
                ...
                <p className="text-slate-500 font-medium">Aucun utilisateur enregistré.</p>
              </div>
            ) : (
        ...
        <section>
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="hidden md:flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-4 py-2.5 shadow-sm focus-within:ring-2 focus-within:ring-indigo-500/20 focus-within:border-indigo-500 transition-all">
              <Search className="w-4 h-4 text-slate-400" />
              <input
                type="text"
                placeholder="Rechercher une fiche..."
                className="text-sm outline-none text-slate-700 placeholder-slate-400 w-56 bg-transparent"
              />
            </div>
          </div>
          ...
          {allEntities.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-dashed border-slate-300 text-center">
              ...
              <h3 className="text-slate-900 text-lg font-semibold mb-2">C'est un peu vide ici</h3>
              <p className="text-slate-500 max-w-sm mx-auto">Commencez par ajouter des clients ou des prospects pour voir apparaître vos fiches ici.</p>
            </div>
          ) : (
...
function StatCard({ title, value, icon, gradient, trend, isAlert }: StatCardProps) {
  return (
    <div className="relative overflow-hidden bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
      <div className="flex items-start justify-between mb-4 relative z-10">
        <div className={`p-3 rounded-2xl bg-gradient-to-br ${gradient} shadow-lg shadow-indigo-500/20`}>
          {icon}
        </div>
        {trend && (
          <div className="flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-100">
