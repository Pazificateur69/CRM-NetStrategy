...
const CustomToolbar = (toolbar: any) => {
    ...
    const label = () => {
        ...
        return (
            <span className="capitalize text-2xl font-bold text-slate-900 dark:text-white tracking-tight">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between mb-8 gap-6">
            <div className="flex items-center gap-6">
                <div className="flex items-center gap-2">
                    <button
                        onClick={goToBack}
                        className="p-2.5 hover:bg-white dark:hover:bg-slate-800 rounded-full text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white border border-transparent hover:border-slate-200 dark:hover:border-slate-700 transition-all shadow-sm hover:shadow"
                    >
                        <ChevronLeft className="w-5 h-5" />
                    </button>
                    <button
                        onClick={goToCurrent}
                        className="px-4 py-2 text-sm font-bold text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded-xl transition-all shadow-sm"
                    >
                        Aujourd'hui
                    </button>
                    <button
            ...
            <div className="flex bg-slate-100 dark:bg-slate-800 p-1.5 rounded-xl border border-slate-200 dark:border-slate-700">
                {['month', 'week', 'day', 'agenda'].map(view => (
                    <button
                        ...
                        className={`px-4 py-2 text-xs font-bold rounded-lg transition-all ${toolbar.view === view
...
const CustomEvent = ({ event }: { event: Event }) => {
    ...
    if (event.type === 'call') { Icon = Phone; bgIcon = 'text-cyan-500'; }
    if (event.type === 'visio') { Icon = Video; bgIcon = 'text-cyan-500'; }
    if (event.type === 'deadline') { Icon = AlertCircle; bgIcon = 'text-orange-500'; }
    if (event.type === 'tournage') { Icon = Video; bgIcon = 'text-rose-500'; }
    if (event.type === 'todo') { Icon = CheckCircle2; bgIcon = 'text-green-500'; }

    return (
        <div className="h-full w-full flex flex-col px-2 py-1.5 overflow-hidden font-medium relative group">
            <div className="flex items-center gap-1.5 mb-0.5">
                <Icon className="w-3.5 h-3.5 opacity-90" />
                <span className="text-[11px] font-bold truncate leading-tight">{event.title}</span>
            </div>
            {(event.client || event.prospect) && (
                <div className="text-[10px] opacity-80 truncate flex items-center gap-1 mt-0.5 font-normal">
                    <User className="w-2.5 h-2.5" />
                    {event.client ? (
                        event.client.societe || event.client.gerant
                    ) : (
                        event.prospect?.societe || event.prospect?.contact
                    )}
                </div>
            )}
        </div>
...
export default function CalendarPage() {
    ...
    const onEventDrop = useCallback(async ({ event, start, end }: any) => {
        ...
        try {
            if (idStr.startsWith('todo-')) {
                const realId = idStr.replace('todo-', '');
                await api.put(`/todos/${realId}`, { date_echeance: start.toISOString() });
                toast.success('Tâche reprogrammée');
            } else if (idStr.startsWith('rappel-')) {
                const realId = idStr.replace('rappel-', '');
                await api.put(`/rappels/${realId}`, { date_rappel: start.toISOString() });
    ...
    const onEventResize = useCallback(async ({ event, start, end }: any) => {
        ...
        // Todos and Rappels generally don't have adjustable duration in this context
        if (idStr.startsWith('todo-') || idStr.startsWith('rappel-')) {
            return;
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Check if we are editing a Todo or Rappel (ReadOnly in this modal for now to prevent complexity)
        if (selectedEvent?.id?.toString().startsWith('todo-')) {
            toast.info("Veuillez modifier la tâche depuis la section Tâches.");
            return;
        }
        if (selectedEvent?.id?.toString().startsWith('rappel-')) {
            toast.info("Veuillez modifier le rappel depuis la section Rappels.");
            return;
        }
    ...
    const handleDelete = async () => {
        ...
        try {
            if (idStr.startsWith('todo-')) {
                const realId = idStr.replace('todo-', '');
                await api.delete(`/todos/${realId}`);
            } else if (idStr.startsWith('rappel-')) {
                const realId = idStr.replace('rappel-', '');
                await api.delete(`/rappels/${realId}`);
            } else {
